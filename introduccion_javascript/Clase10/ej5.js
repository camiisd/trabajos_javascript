//1. Declara y asigna un valor numérico a dos variables: primerNumero y segundoNumero.
let primerNumero = 16;
let segundoNumero = 2;

// 2. Declara las siguientes variables: resultadoSuma, resultadoResta, resultadoMultiplicacion, resultadoDivision.
let resultadoSuma
let resultadoResta
let resultadoMultiplicación
let resultadoDivision

//3. Asigna a cada una de estas variables el resultado de la suma, resta, multiplicación y división de las variables primerNumero y segundoNumero.

resultadoSuma = primerNumero + segundoNumero
resultadoResta = primerNumero - segundoNumero
resultadoMultiplicación = primerNumero * segundoNumero
resultadoDivision = primerNumero / segundoNumero

// Mostrar Resultados: 1. Utiliza console.log(nombreDeLaVariable) para imprimir cada uno de los resultados y ver cómo se ven los mismos.

console.log('El primer número es ' + primerNumero);
console.log('El segundo número es ' + segundoNumero);
console.log('La suma de ambos números es igual a: '+resultadoSuma);
console.log('La resta es igual a: ' + resultadoResta);
console.log('La multiplicación es igual a: ' + resultadoMultiplicación);
console.log('La división es igual a: '+resultadoDivision);

/*Pista 1: Recuerda que para crear una variable utilizamos la palabra "let" seguida del nombre al cual vamos a hacer referencia.
Pista 2: Para asignar el valor a una variable utilizamos el operador "=" seguido del valor que queremos asignarle.*/
