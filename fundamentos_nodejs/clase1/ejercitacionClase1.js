...
let texto = 'Hola Mundo' //Declaro la frase a usar

function contarVocales(texto) { //Declaro la función
    let contador = 0; //Inicializo el contador en 0
    let vocales = ['a', 'e', 'i', 'o', 'u']; //Guardo en una variable las letras a utilizar (vocales)
    for(let i=0; i<texto.length; i++) {//Utilizo un for () para recorrer el array texto
        const minusculas = texto[i].toLowerCase();//Cada letra se convierte en minúsculas si o si
        if (vocales.includes(minusculas)) {//Si la variable vocales coincide con minisculas,
            contador +=1 //Se suma +1 al contador
        }
    }
    return contador;//Cuando se recorre todo el array devolvemos el resultado final del contador

}

console.log(contarVocales(texto))//Imprimimos el resultado
...
function contarLetra(texto, letra) {
    let contador = 0;//Inicializo una variable con el contador
    for(let i=0; i < texto.length; i++) {//Con for recorro el string
        let posicion = texto[i];//Asigno una variable con la posición en i para luego trabajar con el contenido de ese string
        if (posicion === letra){
            contador++
        };
    };
...
function obtenerNumerosUnicos (numeros) {
   let nuevosNumeros = new Set (numeros);
   let numerosUnicos = [...nuevosNumeros];
...
function fraseInversa(frase) {
    let separarPalabras = frase.split(' ');
    separarPalabras.reverse();
    let fraseInversa = separarPalabras.join(' ');
    return fraseInversa; 
};

console.log(fraseInversa(frase));